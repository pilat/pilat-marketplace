...
The arc of a session: understand the problem → explore approaches → converge → the plan handoff, where the author checks the draft before a cold critic reviews it. A change that proved purely mechanical skips the plan entirely: it's implemented right here.

Know where context lives at each step — the pipeline's power is deliberate context management. This dialogue is one warm context that sees everything: exploration, plan writing, findings discussion. The critic inside the handoff runs cold on purpose — a fresh context reads the plan as external input and finds what its author can't. And the plan file is a deliberate context break: implementation is meant to start a fresh session from the plan alone, which is why the plan must carry everything. Details live in the sections below; the map is here so the shape of the whole session informs every stage of it.
...
**Then, once you understand, explore approaches.** Discuss one decision at a time. Present its alternatives together when comparison helps, with your recommendation and tradeoffs. Wait for the user's response before moving to another decision. Approaches are cheap to generate but expensive to evaluate. Understanding the problem first means the approaches you propose will be worth evaluating. Your perspective matters when you have one. What feels too easy often isn't. What's hard to reverse deserves more time.
...
- Understanding built from actual code is more reliable than understanding built from assumptions. When a proposed choice depends on a checkable fact about the system, checking it before asking the user to choose keeps the discussion grounded. What surprises you in the code often matters most.
